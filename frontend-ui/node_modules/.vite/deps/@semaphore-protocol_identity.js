...
function isBuffer2(value) {
  return import_buffer3.Buffer.isBuffer(value);
}
...
var typeChecks = Object.freeze({
  __proto__: null,
  isArray: isArray2,
  isBigInt: isBigInt2,
  isBigNumber: isBigNumber2,
  isBigNumberish: isBigNumberish2,
  isBuffer: isBuffer2,
  isDefined: isDefined2,
  isFunction: isFunction2,
  isHexadecimal: isHexadecimal2,
  isNumber: isNumber2,
  isObject: isObject2,
  isString: isString3,
  isStringifiedBigInt: isStringifiedBigInt2,
  isSupportedType: isSupportedType2,
  isType: isType2,
  isUint8Array: isUint8Array2,
  supportedTypes: supportedTypes2
});
function requireDefined(parameterValue, parameterName) {
  if (!isDefined2(parameterValue)) {
...
function requireUint8Array(parameterValue, parameterName) {
  if (!isUint8Array2(parameterValue)) {
...
function requireStringifiedBigInt(parameterValue, parameterName) {
  if (!isStringifiedBigInt2(parameterValue)) {
    throw new TypeError(`Parameter '${parameterName}' is not a stringified bigint`);
  }
}
function requireHexadecimal2(parameterValue, parameterName, prefix = true) {
  if (!isHexadecimal2(parameterValue, prefix)) {
...
function requireBigNumber(parameterValue, parameterName) {
  if (!isBigNumber2(parameterValue)) {
...
var errorHandlers = Object.freeze({
  __proto__: null,
  requireArray,
  requireBigInt: requireBigInt2,
  requireBigNumber,
  requireBigNumberish: requireBigNumberish2,
  requireBuffer,
  requireDefined,
  requireFunction,
  requireHexadecimal: requireHexadecimal2,
  requireNumber,
  requireObject,
  requireString: requireString3,
  requireStringifiedBigInt,
  requireTypes: requireTypes3,
  requireUint8Array
});
...
function hexadecimalToBigInt(value) {
  if (!isHexadecimal2(value) && !isHexadecimal2(value, false)) {
...
function beBufferToBigInt(value) {
  ...
  return BigInt(`0x${import_buffer3.Buffer.from(value).toString("hex")}`);
}
function leBufferToBigInt2(value) {
  requireTypes3(value, "value", ["Buffer", "Uint8Array"]);
  return BigInt(`0x${import_buffer3.Buffer.from(value).reverse().toString("hex")}`);
}
...
function beBigIntToBuffer(value, size) {
  ...
  const buffer = import_buffer3.Buffer.alloc(size, 0);
  const fromHex = import_buffer3.Buffer.from(hex, "hex");
...
function leBigIntToBuffer2(value, size) {
  ...
  const buffer = import_buffer3.Buffer.alloc(size, 0);
  const fromHex = import_buffer3.Buffer.from(hex, "hex").reverse();
  fromHex.copy(buffer, 0);
  return buffer;
}
...
function hexadecimalToBuffer(value) {
  requireHexadecimal2(value, "value", false);
  if (value.length % 2 !== 0) {
    value = `0${value}`;
  }
  return import_buffer3.Buffer.from(value, "hex");
}
function bufferToHexadecimal(value) {
  requireTypes3(value, "value", ["Buffer", "Uint8Array"]);
  return import_buffer3.Buffer.from(value).toString("hex");
}
function bufferToBase64(value) {
  requireTypes3(value, "value", ["Buffer", "Uint8Array"]);
  return import_buffer3.Buffer.from(value).toString("base64");
}
function base64ToBuffer(value) {
  requireString3(value, "value");
  return import_buffer3.Buffer.from(value, "base64");
}
function textToBase64(value) {
  requireString3(value, "value");
  return import_buffer3.Buffer.from(value, "utf8").toString("base64");
}
function base64ToText(value) {
  requireString3(value, "value");
  return import_buffer3.Buffer.from(value, "base64").toString("utf8");
}
var conversions = Object.freeze({
  ...
  beBufferToBigInt,
  bigIntToBuffer,
  bigIntToHexadecimal: bigIntToHexadecimal2,
  ...
  hexadecimalToBigInt,
  hexadecimalToBuffer,
  leBigIntToBuffer: leBigIntToBuffer2,
  leBufferToBigInt: leBufferToBigInt2,
  textToBase64
});
...
var proofPacking = Object.freeze({
  __proto__: null,
  packGroth16Proof,
  unpackGroth16Proof
});
...
function requireBigInt3(parameterValue, parameterName) {
  if (!isBigInt3(parameterValue)) {
    throw new TypeError(`Parameter '${parameterName}' is not a bigint, received type: ${typeof parameterValue}`);
  }
}
function requireBigNumberish3(parameterValue, parameterName) {
  if (!isBigNumberish3(parameterValue)) {
    throw new TypeError(`Parameter '${parameterName}' is not a bignumber-ish`);
  }
}
function requireTypes4(parameterValue, parameterName, types) {
  for (const type of types) {
...
// node_modules/@zk-kit/eddsa-poseidon/node_modules/@zk-kit/utils/dist/lib.esm/conversions.js
function bigIntToHexadecimal3(value) {
  requireBigInt3(value, "value");
  let hex = value.toString(16);
  if (hex.length % 2 !== 0) {
    hex = `0${hex}`;
  }
...
function beBufferToBigInt2(value) {
  requireTypes4(value, "value", ["Buffer", "Uint8Array"]);
  return BigInt(`0x${import_buffer6.Buffer.from(value).toString("hex")}`);
}
function leBufferToBigInt3(value) {
  requireTypes4(value, "value", ["Buffer", "Uint8Array"]);
  return BigInt(`0x${import_buffer6.Buffer.from(value).reverse().toString("hex")}`);
}
function bufferToBigInt2(value) {
  return beBufferToBigInt2(value);
}
function leBigIntToBuffer3(value, size) {
  const hex = bigIntToHexadecimal3(value);
  const minSize = Math.ceil(hex.length / 2);
  if (!size) {
    size = minSize;
  } else if (size < minSize) {
...
function bigNumberishToBigInt2(value) {
  requireBigNumberish3(value, "value");
  if (isBuffer3(value) || isUint8Array3(value)) {
    return bufferToBigInt2(value);
  }
  return BigInt(value);
}
...
var import_poseidon5 = __toESM(require_poseidon5());
...
function isPoint(point) {
  return isArray3(point) && point.length === 2 && isBigNumber3(point[0]) && isBigNumber3(point[1]);
}
function isSignature(signature) {
  return isObject3(signature) && Object.prototype.hasOwnProperty.call(signature, "R8") && Object.prototype.hasOwnProperty.call(signature, "S") && isPoint(signature.R8) && isBigNumber3(signature.S);
}
function checkPrivateKey(privateKey) {
  requireTypes4(privateKey, "privateKey", ["Buffer", "Uint8Array", "string"]);
  return import_buffer7.Buffer.from(privateKey);
}
function checkMessage(message) {
  requireTypes4(message, "message", ["bignumberish", "string"]);
  if (isBigNumberish3(message)) {
    return bigNumberishToBigInt2(message);
  }
  return bufferToBigInt2(import_buffer7.Buffer.from(message));
}
...
function deriveSecretScalar(privateKey) {
  ...
  return shiftRight3(leBufferToBigInt3(hash$1), BigInt(3)) % subOrder;
}
...
function signMessage(privateKey, message) {
  ...
  const s = leBufferToBigInt3(sBuff);
  const A = mulPointEscalar(Base8, shiftRight3(s, BigInt(3)));
  const msgBuff = leBigIntToBuffer3(message, 32);
  const rBuff = hash(import_buffer7.Buffer.concat([hash$1.slice(32, 64), msgBuff]));
  const Fr2 = new F1Field2(subOrder);
  const r2 = Fr2.e(leBufferToBigInt3(rBuff));
...
function verifySignature(message, signature, publicKey) {
  ...
  let pRight = mulPointEscalar(_publicKey, mul2(hm, BigInt(8)));
...
function bufferToBase642(value) {
  requireTypes(value, "value", ["Buffer", "Uint8Array"]);
  return import_buffer8.Buffer.from(value).toString("base64");
}
function base64ToBuffer2(value) {
  requireString(value, "value");
  return import_buffer8.Buffer.from(value, "base64");
}
function textToBase642(value) {
  requireString(value, "value");
  return import_buffer8.Buffer.from(value, "utf8").toString("base64");
}
...
var Identity = class _Identity {
  ...
  export() {
    if (isString(this._privateKey)) {
      return textToBase642(this._privateKey);
    }
    return bufferToBase642(this.privateKey);
  }
  /**
  ...
  static import(privateKey) {
    return new _Identity(base64ToBuffer2(privateKey));
  }
  /**
