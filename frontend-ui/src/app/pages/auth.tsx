...
// function ZkProofAnimation({ active, stepIdx }: { active: boolean; stepIdx: number }) {
//   if (!active) return null;
//   return (
//     <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
//       className="mt-4 rounded-xl border border-red-100 bg-red-50/60 p-4">
...
//               i < stepIdx ? "text-green-700 line-through opacity-60"
//               : i === stepIdx ? "text-red-700 font-medium"
//               : "text-gray-400"
//             }`}>{step}</span>
//           </div>
//         ))}
...
//   const handleVerify = async () => {
//     try {
//       setStatus({ type: "loading", text: "Starting proof generation…" });
//       setIsProving(true);
//       setProofStep(0);
//       await verifyProofAndCreateSession((step) => setProofStep(step));
//       setProofStep(5); // all done
//       setStatus({ type: "success", text: "Session created! Redirecting…" });
//       refresh();
//       setTimeout(() => navigate("/marketplace"), 900);
//     } catch (e) {
...
//                       {isProving ? <><Loader2 className="w-4 h-4 animate-spin" />Proving membership…</> : <><KeyRound className="w-4 h-4" />Verify & Get Session</>}
//                     </Button>
//                     <AnimatePresence><ZkProofAnimation active={isProving} stepIdx={proofStep} /></AnimatePresence>
//                     <StatusBanner status={status} />
...
// ── ZK Animation driven by real proof step callback ──────────────────────
function ZkProofAnimation({ active, stepIdx }: { active: boolean; stepIdx: number }) {
  if (!active) return null;
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
...
function GoogleLogo() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908C16.618 14.013 17.64 11.705 17.64 9.2z" fill="#4285F4"/>
      <path d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z" fill="#34A853"/>
      <path d="M3.964 10.71c-.18-.54-.282-1.117-.282-1.71s.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/>
      <path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 6.29C4.672 4.163 6.656 3.58 9 3.58z" fill="#EA4335"/>
    </svg>
...
export function AuthPage() {
  ...
  // If already enrolled but no session, jump to verify
  useEffect(() => {
    if (!hasSession && isEnrolled()) setStep("verify");
  }, [hasSession]);
  ...
  // useGoogleLogin uses the Authorization Code flow — it fetches the access
  // token and then we exchange it for an id_token via Google's tokeninfo endpoint.
  // We use the implicit flow (responseType: 'token') to get id_token directly.
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        setGoogleLoading(true);
        setStatus({ type: "loading", text: "Verifying USC identity with Google…" });

        // Exchange access token for user info to get id_token
        // For Semaphore path we need to enroll then verify separately.
        // We use the access_token to call userinfo and get the email for USC check.
        ...
        if (!userInfo.email?.toLowerCase().endsWith("@usc.edu")) {
          setStatus({ type: "error", text: "Please sign in with your @usc.edu Google account." });
          setGoogleLoading(false);
          return;
        }
        if (!userInfo.email_verified) {
          setStatus({ type: "error", text: "Your USC email is not verified with Google." });
          setGoogleLoading(false);
          return;
        }

        // Enroll using the access token (backend verifies via Google's tokeninfo)
        setStatus({ type: "loading", text: "Creating anonymous identity…" });
        await enrollWithGoogle(tokenResponse.access_token);

        setStatus({ type: "success", text: "USC identity verified! Generating ZK proof…" });
        refresh();

        // Move to verify step and auto-trigger proof generation
        setStep("verify");
        setGoogleLoading(false);
      } catch (e) {
        setStatus({ type: "error", text: `Google sign-in failed: ${(e as Error).message}` });
        setGoogleLoading(false);
      }
    },
    onError: (err) => {
      console.error("Google OAuth error", err);
      setStatus({ type: "error", text: "Google sign-in was cancelled or failed. Please try again." });
      setGoogleLoading(false);
    },
  ...
  const handleDevEnroll = async () => {
    if (!devEmail.trim()) { setStatus({ type: "error", text: "Enter a @usc.edu email." }); return; }
    if (!devEmail.toLowerCase().endsWith("@usc.edu")) {
      setStatus({ type: "error", text: "Must be a @usc.edu email address." }); return;
    }
    try {
      setStatus({ type: "loading", text: "Creating Semaphore identity…" });
      await enrollWithEmail(devEmail.trim());
  ...
  const handleRecover = async () => {
    ...
    try {
      setStatus({ type: "loading", text: "Recovering identity…" });
      await recoverIdentityWithPassword(recoverPwd.trim());
      setStatus({ type: "success", text: "Identity recovered!" });
      refresh();
      setRecoverPwd("");
      setTimeout(() => setStep("verify"), 700);
    } catch (e) {
  ...
  const googleConfigured = !!googleClientId && googleClientId.length > 10;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-red-50/30 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md">
                ...
                <Card className="border-0 shadow-xl shadow-gray-200/60">
                  <CardContent className="pt-8 pb-8 px-8">
                    <div className="text-center mb-8">
                      ...
                      <h1 className="text-2xl font-bold text-gray-900 mb-2">Access ShopSC</h1>
                      <p className="text-gray-500 text-sm leading-relaxed">Verify your USC enrollment anonymously. Your email is never stored or shared.</p>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {googleConfigured ? (
                        <button
                          onClick={() => { resetStatus(); handleGoogleLogin(); }}
                          disabled={googleLoading}
                          className="w-full flex items-center justify-center gap-3 border border-gray-300 rounded-lg px-4 py-3 text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                          {googleLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <GoogleLogo />}
                          {googleLoading ? "Connecting to Google…" : "Continue with Google (@usc.edu)"}
                        </button>
                      ) : (
                        /* Google not configured — show grayed out button with tooltip */
                        <div className="relative group">
                          <button
                            disabled
                            className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-lg px-4 py-3 text-sm font-medium text-gray-400 bg-gray-50 cursor-not-allowed"
                          >
                            <GoogleLogo />
                            Continue with Google (@usc.edu)
                          </button>
                          <div className="absolute bottom-full left-0 right-0 mb-2 hidden group-hover:block">
                            <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 text-center">
                              Set VITE_GOOGLE_CLIENT_ID in .env.local to enable Google sign-in
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="relative flex items-center gap-3">
                        ...
                        <span className="text-xs text-gray-400">or for development</span>
                        <div className="flex-1 h-px bg-gray-200" />
                      </div>

                      <Button variant="outline" className="w-full gap-2 text-gray-600" onClick={() => { setStep("enroll"); resetStatus(); }}>
                        <KeyRound className="w-4 h-4" />
                        Dev email enroll
                      </Button>

                      <Button variant="ghost" className="w-full text-sm text-gray-400 gap-2" onClick={() => { setStep("recover"); resetStatus(); }}>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-100">
                      ...
                      <div className="space-y-2">
                        {[
                          "Sign in with your @usc.edu Google account",
                          "A ZK proof is generated entirely in your browser",
                          "Backend verifies the proof — your email is never sent",
                        ].map((text, i) => (
                          <div key={i} className="flex items-start gap-2.5 text-xs text-gray-500">
                            <span className="w-4 h-4 rounded-full bg-red-100 text-red-600 font-bold text-[10px] flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
                            {text}
            ...
            {step === "enroll" && (
              <motion.div key="enroll" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <Card className="border-0 shadow-xl shadow-gray-200/60">
                  <CardContent className="pt-8 pb-8 px-8">
                    <button onClick={() => { setStep("welcome"); resetStatus(); }} className="text-xs text-gray-400 hover:text-gray-600 mb-4 flex items-center gap-1">← Back</button>
                    <div className="mb-2 inline-flex items-center gap-1.5 bg-yellow-50 border border-yellow-200 text-yellow-700 text-xs px-2.5 py-1 rounded-full">
                      <AlertCircle className="w-3 h-3" /> Development only
                    </div>
                    <div className="mb-6 mt-2">
                      <h2 className="text-xl font-bold text-gray-900 mb-1">Dev Enrollment</h2>
                      <p className="text-sm text-gray-500">Enter a @usc.edu email to generate your anonymous identity. Use this only for local development — in production use Google sign-in.</p>
                    </div>
                    <div className="space-y-3">
                      <Input
                        type="email"
                      ...
                      <Button onClick={handleDevEnroll} disabled={status.type === "loading"} className="w-full bg-red-600 hover:bg-red-700 gap-2">
                        {status.type === "loading"
                          ...
                          : <><Shield className="w-4 h-4" />Enroll Anonymously</>}
                      </Button>
                    </div>
                    <StatusBanner status={status} />
                    <div className="mt-4 pt-4 border-t border-gray-100">
              ...
              <motion.div key="verify" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <Card className="border-0 shadow-xl shadow-gray-200/60">
                  <CardContent className="pt-8 pb-8 px-8">
                    ...
                    <Button onClick={handleVerify} disabled={isProving} className="w-full bg-red-600 hover:bg-red-700 gap-2 mb-3" size="lg">
                      {isProving
                        ? <><Loader2 className="w-4 h-4 animate-spin" />Proving membership…</>
                        : <><KeyRound className="w-4 h-4" />Verify & Get Session</>}
                    </Button>
                    <AnimatePresence><ZkProofAnimation active={isProving} stepIdx={proofStep} /></AnimatePresence>
                    <StatusBanner status={status} />
                    ...
                    {/* Backup — highlighted warning */}
                    <div className="mt-6 pt-5 border-t border-gray-100">
                      <div className="flex items-start gap-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg mb-3">
                        <AlertCircle className="w-4 h-4 text-yellow-600 flex-shrink-0 mt-0.5" />
                        <div>
                          <p className="text-xs font-semibold text-yellow-800">Back up your identity</p>
                          <p className="text-xs text-yellow-700 mt-0.5">
                            Your anonymous ID lives only in this browser. Clear your browser data or switch devices and you lose access to your listings — unless you back up now.
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                ...
                <Card className="border-0 shadow-xl shadow-gray-200/60">
                  <CardContent className="pt-8 pb-8 px-8">
                    ...
                    <div className="space-y-3">
                      <div className="relative">
                        <Input
                          type={showRecoverPwd ? "text" : "password"}
                      ...
                      <Button onClick={handleRecover} disabled={status.type === "loading"} className="w-full gap-2">
                        {status.type === "loading"
                          ...
                          : <><RefreshCw className="w-4 h-4" />Recover Identity</>}
                      </Button>
                    </div>
                    <StatusBanner status={status} />
                    <div className="mt-4 pt-4 border-t border-gray-100">
          ...
          <p className="text-center text-xs text-gray-400 mt-6 leading-relaxed">
            Your email is verified but never stored.<br />All ZK proofs run locally in your browser.
          </p>
        </div>
      </div>
    </div>
