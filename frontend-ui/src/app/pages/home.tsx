          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-gray-400 mb-6 leading-relaxed">
                ShopSC uses zk-SNARKs to verify you're a USC student without ever seeing your email. 
                Your Google token never leaves your browser — only a cryptographic proof is sent.
              </p>
              <div className="space-y-3">
                {[
                  "Email verified in-circuit, never transmitted",
                  "Stable pseudonym derived from your identity commitment",
                  "No PII in database — only anonymous IDs",
                  "Cross-session identity via encrypted recovery",
                ].map((item, i) => (
            ...
            <div className="relative">
              <div className="bg-gray-800/60 border border-gray-700 rounded-2xl p-6 font-mono text-sm">
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <div className="w-3 h-3 rounded-full bg-yellow-500" />
                  <div className="w-3 h-3 rounded-full bg-green-500" />
                  <span className="ml-2 text-gray-400 text-xs">ZK Auth Flow</span>
                </div>
                <div className="space-y-2 text-xs leading-relaxed">
                  <div className="text-gray-400">{"// 1. Client generates identity"}</div>
                  <div className="text-green-400">identity <span className="text-gray-400">=</span> new Identity()</div>
                  <div className="mt-3 text-gray-400">{"// 2. ZK proof in browser"}</div>
                  <div className="text-blue-400">proof <span className="text-gray-400">=</span> <span className="text-yellow-400">generateProof</span>(</div>
                  <div className="pl-4 text-gray-300">identity, group, signal</div>
                  <div className="text-blue-400">)</div>
                  <div className="mt-3 text-gray-400">{"// 3. Send ONLY the proof"}</div>
                  <div className="text-purple-400">POST /auth/verify-proof</div>
                  <div className="pl-4 text-gray-400">{"{ proof, stableId }"}</div>
                  <div className="mt-3 text-gray-400">{"// ✓ No email. No sub. No PII."}</div>
                  <div className="text-green-400">{"← { token: jwt }"}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Link to={hasSession ? "/create-listing" : "/auth"}>
            <Button size="lg" variant="secondary" className="gap-2">
              {hasSession ? "Create Your First Listing" : "Join the Marketplace"}
              <ArrowRight className="w-5 h-5" />
            </Button>
          </Link>
        </div>
...
// Note: HomePage is defined above. The auth-aware version wraps CTAs.
// The existing export is sufficient; auth-gating is handled in navigation + route guards.
